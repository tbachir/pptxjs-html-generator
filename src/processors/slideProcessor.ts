...
export function genTable(tableNode: any, graphicFrameNode: any, warpObj: WarpObj): string {
...
export function genChart(chartNode: any, graphicFrameNode: any, warpObj: WarpObj): string {
  const xfrmNode = getTextByPathList(graphicFrameNode, ["p:xfrm"]);
  const position = getPosition(xfrmNode, graphicFrameNode);
  const size = getSize(xfrmNode);
...
export function genDiagram(diagramNode: any, warpObj: WarpObj, source: string, sType: string): string {
...
export function genCustomShape(
  node: any,
  pNode: any,
  slideLayoutSpNode: any,
  slideMasterSpNode: any,
...
// Helper functions
function getTextDirection(pPr: any): string {
  const rtl = getTextByPathList(pPr, ["attrs", "rtl"]);
  return rtl === "1" ? "rtl" : "ltr";
}

function parseColor(solidFill: any, warpObj: WarpObj): string | null {
